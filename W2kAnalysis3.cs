...
{
    public class W2kAnalysis3 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                // --- New User-Configurable Properties ---
                Strength = 12;
                DaysToLoad = 14;
        ...
        private void ProcessHistoricalData()
        {
            ...
            // --- Calculate the analysis date range ---
            ...
            nextApexFlashBar = CurrentBar + 240; // Approx 2 minutes on a 30-sec chart
        ...
        {
            if (isDataProcessed && CurrentBar >= nextReminderBar && reminderFlashCounter <= 0) { reminderFlashCounter = 30; nextReminderBar = CurrentBar + random.Next(240, 601); }
            if (isDataProcessed && CurrentBar >= nextApexFlashBar && apexFlashCounter <= 0) { apexFlashCounter = 30; nextApexFlashBar = CurrentBar + 240; }
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private W2kAnalysis3[] cacheW2kAnalysis3;
		public W2kAnalysis3 W2kAnalysis3(int strength, int daysToLoad, TimeSpan session1StartTime, TimeSpan session1EndTime, bool enableSession2, TimeSpan session2StartTime, TimeSpan session2EndTime, bool enableSession3, TimeSpan session3StartTime, TimeSpan session3EndTime)
		{
			return W2kAnalysis3(Input, strength, daysToLoad, session1StartTime, session1EndTime, enableSession2, session2StartTime, session2EndTime, enableSession3, session3StartTime, session3EndTime);
		}

		public W2kAnalysis3 W2kAnalysis3(ISeries<double> input, int strength, int daysToLoad, TimeSpan session1StartTime, TimeSpan session1EndTime, bool enableSession2, TimeSpan session2StartTime, TimeSpan session2EndTime, bool enableSession3, TimeSpan session3StartTime, TimeSpan session3EndTime)
		{
			if (cacheW2kAnalysis3 != null)
				for (int idx = 0; idx < cacheW2kAnalysis3.Length; idx++)
					if (cacheW2kAnalysis3[idx] != null && cacheW2kAnalysis3[idx].Strength == strength && cacheW2kAnalysis3[idx].DaysToLoad == daysToLoad && cacheW2kAnalysis3[idx].Session1StartTime == session1StartTime && cacheW2kAnalysis3[idx].Session1EndTime == session1EndTime && cacheW2kAnalysis3[idx].EnableSession2 == enableSession2 && cacheW2kAnalysis3[idx].Session2StartTime == session2StartTime && cacheW2kAnalysis3[idx].Session2EndTime == session2EndTime && cacheW2kAnalysis3[idx].EnableSession3 == enableSession3 && cacheW2kAnalysis3[idx].Session3StartTime == session3StartTime && cacheW2kAnalysis3[idx].Session3EndTime == session3EndTime && cacheW2kAnalysis3[idx].EqualsInput(input))
						return cacheW2kAnalysis3[idx];
			return CacheIndicator<W2kAnalysis3>(new W2kAnalysis3(){ Strength = strength, DaysToLoad = daysToLoad, Session1StartTime = session1StartTime, Session1EndTime = session1EndTime, EnableSession2 = enableSession2, Session2StartTime = session2StartTime, Session2EndTime = session2EndTime, EnableSession3 = enableSession3, Session3StartTime = session3StartTime, Session3EndTime = session3EndTime }, input, ref cacheW2kAnalysis3);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.W2kAnalysis3 W2kAnalysis3(int strength, int daysToLoad, TimeSpan session1StartTime, TimeSpan session1EndTime, bool enableSession2, TimeSpan session2StartTime, TimeSpan session2EndTime, bool enableSession3, TimeSpan session3StartTime, TimeSpan session3EndTime)
		{
			return indicator.W2kAnalysis3(Input, strength, daysToLoad, session1StartTime, session1EndTime, enableSession2, session2StartTime, session2EndTime, enableSession3, session3StartTime, session3EndTime);
		}

		public Indicators.W2kAnalysis3 W2kAnalysis3(ISeries<double> input , int strength, int daysToLoad, TimeSpan session1StartTime, TimeSpan session1EndTime, bool enableSession2, TimeSpan session2StartTime, TimeSpan session2EndTime, bool enableSession3, TimeSpan session3StartTime, TimeSpan session3EndTime)
		{
			return indicator.W2kAnalysis3(input, strength, daysToLoad, session1StartTime, session1EndTime, enableSession2, session2StartTime, session2EndTime, enableSession3, session3StartTime, session3EndTime);
		}
	}
